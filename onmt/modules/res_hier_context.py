""" Residual Hierarquical Context module """
import torch
...
class ResHierarquicalContext(nn.Module):
    """
    Residual Hierarquical Context module of HAN.
    ...
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
        ...
        # self.layer_norm_query_word = nn.LayerNorm(d_model, eps=1e-6)  # NOTE
        # self.layer_norm_query_sent = nn.LayerNorm(d_model, eps=1e-6)  # NOTE

        self.layer_norm_query = nn.LayerNorm(d_model, eps=1e-6)  # NOTE: maybe dropable
        self.layer_norm_ctx = nn.LayerNorm(d_model, eps=1e-6)  # NOTE: maybe dropable
    ...
    def forward(self, query, ctxs, contexts):
        ...
        query_word_norm = self.layer_norm_query(query)
        ...
        for ctx, context in zip(ctxs, contexts):
            ...
            # NOTE: LN:
            context_norm = self.layer_norm_ctx(context_)
            context_word_, attn_word_ = self.word_level_attn(
                query_word_norm, context_norm, ctx_word_mask_)
            # NOTE: Why not res connect?
            context_word_ = self.dropout(context_word_) + query
            # NOTE like: context_word_ = self.drop(context_word_) + query
        ...
        context_word_norm = self.layer_norm_word(context_word)
        context_sent, attn_sent = self.sent_level_attn(
            query_word_norm, context_word_norm, ctx_sent_mask)
        context_sent = self.dropout(context_sent) + query
        out = self.feed_forward(context_sent)
    ...
    def word_level_attn(self, query, context, ctx_word_mask):
        ...
        # NOTE 1: layernorm context? : query is LNed but not context !
        # NOTE 2: (if consider input LNed), then query LNed*2 while context*1
    ...
    def sent_level_attn(self, query, context_word, ctx_sent_mask):
        ...
        context_word = context_word.transpose(1, 2).contiguous()  # (B, Lq, nc, D)
        context_word = context_word.view(b_size*len_q, n_ctx, dim)  # (B*Lq, nc, D)

        # query_sent_norm = self.layer_norm_query_sent(query)  # NOTE: neccessary ?
        b_size_, len_q_, dim_ = query.size()
        # aeq(b_size_, b_size)
        # aeq(len_q, len_q_)
        query = query.view(b_size_*len_q_, 1, dim_)  # (B*Lq, 1, D)
        ...
        context_sent, attn_sent = self.sent_attn(
            context_word, context_word, query,
    ...
    #     # NOTE: in RNN, after BothContextGate, we have a dropout!
