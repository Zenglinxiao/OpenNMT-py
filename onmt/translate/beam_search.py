...
class BeamSearch(DecodeStrategy):
    ...
    def _pick(self, log_probs):
        ...
        # _B, _, vocab_size = log_probs.shape()
        # step = len(self)
        # if (self.target_prefix is not None and
        #         self.target_prefix.shape(1) < step):
        #     constraint_pick = self.target_prefix[:, step - 1]  # TODO: choose batch index
        #     pass  # mask padding idk
        #     # some reshape, repete to match vocabulary
        #     # apply multiplication to scores in order to make the constraint_pick get biggest score
        #     # scores = fn(scores)
